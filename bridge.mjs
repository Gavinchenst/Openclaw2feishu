...
const clawdConfig = JSON.parse(mustRead(OPENCLAW_CONFIG_PATH, 'OpenClaw config'));
...
function shouldRespondInGroup(text, mentions) {
  ...
  if (/^(alen|openclaw|bot|助手|智能体)[\s,:，：]/i.test(text)) return true;
  return false;
}
  ...
  'im.message.receive_v1': async (data) => {
    try {
      ...
      setImmediate(async () => {
        ...
        let reply = '';
        try {
          reply = await askOpenClaw({ text, sessionKey });
        } catch (e) {
